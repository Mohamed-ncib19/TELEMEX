...
function get_single_reverse_info1($value,$token)
{
   
        try {

            $url = "http://fleet.tn/ws_rimtrack_all/realTimeRecords/".$value;
...
function get_single_status_gps_info($id,$token)
{
    ...
    $datedtimed = date('Y/m/d H:i:s');
    $sql = "SELECT * FROM status_gps,`chargement` as ch where id_chargement = {$id} and Etat=0 and  ch.statutenvoi!=0 and ch.statutenvoi!=2 AND ch.id_charg= {$id} ";
    //    try {
            ...
            if(!empty($vals)){
                ...
                SELECT JSON_OBJECT(
                   ...
                   ,'client'         ,filterval.name_expediteur   
                   ,'adresseclient'  ,filterval.adresse_charg            
                   ,'longitude'         ,filterval.longitude   
                   ,'latitude'  ,filterval.latitude            
                   ,'longitude1'         ,filterval.longitude1   
                   ,'latitude1'  ,filterval.latitude1            
                   ,'name_expediteur'  ,filterval.name_expediteur            
                   ,'telephone_expediteur'  ,filterval.telephone_expediteur            
                   ,'contact_name'  ,filterval.contact_name            
                   ,'contact_phone'  ,filterval.contact_phone            
                   ,'marchandise'  ,filterval.marchandise            
                   ,'prix_prop'  ,filterval.prix_prop            
                   ,'priceestimated'  ,filterval.priceestimated            
                   ,'details_march'  ,filterval.details_march            

                   ,'telephoneclient',filterval.telephone_expediteur  
                        ) AS 'Result' FROM  (
               ...
               SELECT *  FROM abonnements WHERE id_abonnement= {$Id_abonnement} 
                ...
                //    var_dump($resp1);
                    ...
                    $Id_transporteur= $resp1->Abonnement->telephone;
                   ...
                   // echo( "\n 44444444444444444444444444444444444444444444444444444 \n");
                    ...
                    $Id_transporteur1= $resp1->Transporteur->telephone;
                   ...
                   $selectSQLtransporteur2="SELECT JSON_OBJECT(
                    'nom'                      ,filterval. nom  
                   , 'prenom'                   ,filterval. prenom       
                   , 'r_s'                      ,filterval. r_s          
                   , 'pays'                     ,filterval. pays         
                   ...
                   , 'Date_inscri'              ,filterval. Date_inscri  
                        ...
                        SELECT *  FROM transporteur
                        WHERE telephone= {$array->telephone}
                    ) AS filterval; ";
                $query1transporteur2 = $pdo->prepare($selectSQLtransporteur2);
                $query1transporteur2->execute();
                $val1transporteur2 = $query1transporteur2->fetchAll(PDO::FETCH_ASSOC);
                $resp1->Expediteur=json_decode($val1transporteur2[0]["Result"]);
    ...
    //    try {
            ...
            if(!empty($vals)){
                ...
                for($i = 0;$i < count($vals);$i++){
                    ...
                    $resp1->Abonnement=json_decode(  $val['Result']);
                    // //var_dump($val1abonnement[0]);
                  ...
                  // echo( "\n 333333333333333333333333333333333333333333333333333 \n");
                   ...
                   // //var_dump($resp1);
                    ...
                    $resp1->Chargeur=json_decode($val1transporteur[0]['Result']) ;
                   // echo( "\n 555555555555555555555555555555555555555555555555555555555 \n");
                   //// echo( json_encode ($resp1));
                    $response[$i]= $resp1;
                    //var_dump($response);
