...
<div class="form-group">
    ...
    width: 100% !important;">Détail de l’offre</b>
                    ...
                    <div class="col-md-12">Type de véhicule : 
...
?>
    ...
    <tbody>
                                                <?php
                                                // Coordonnées GPS de la position fixe
$lat1 = $res['latitude'];
$lng1 = $res['longitude'];
...
while ($row = mysqli_fetch_array($result)) {
  ...
  $url = "http://fleet.tn/ws_rimtrack_all/realTimeRecords/". $row['ID_gps'];
...
if (is_array($resp1)) {
  ...
  $url = 'https://nominatim.openstreetmap.org/reverse?format=json&lon='.$lng2.'&lat='.$lat2;
  $curl_handle=curl_init();
  curl_setopt($curl_handle, CURLOPT_URL, $url);
  curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 2);
  curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Your application name');
  $posi = curl_exec($curl_handle);
  $posi1=json_decode($posi);
  ...
  echo "<tr>";
  echo "<td><input type='checkbox' name='item[]' value='{$row['ID_gps']}'></td>";
  echo "<td>{$row['ID_gps']}</td>";
  echo "<td>{$row['matricule']}</td>";
  echo "<td>{$posi1->display_name}</td>";
  echo "<td>{$distance} km</td>";
  echo "</tr>";
} elseif (is_object($resp1)) {
    // handle object  
  if (property_exists($resp1, 'errorCode')) {
    echo "<td><input type='checkbox' name='vehicules[]' value='{$row['id_abonnement']}'></td>";
    echo "<td>{$row['ID_gps']}</td>";
    echo "<td>{$row['matricule']}</td>";
    echo "<td>Inconnu</td>";
    echo "<td>Inconnu</td>";
...
<select style="height: 50px;" type="text" id="vehicules" name="devise" required>
                            ...
                            <label class="" style="color: red;"><?php echo $res['statutenvoi']==1?"Ce chargement est deja prise en charge par un transporteur!":"" ?></label>
                            ...
                            <button type="submit" class="btn btn-primary pull-right" <?php echo $res['statutenvoi']==1?"disabled":"" ?>>Attribuer
                            </button>
                            </form>
                        </div>
                    </div>
